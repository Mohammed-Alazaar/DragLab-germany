...
exports.getQualifications = (req, res, next) => {
    // Use the lang already resolved by middleware when possible
    const supportedLangs = allanguages; // ['EN','ES','DE','TR','FR']
    ...
    // Your existing translations object, unchanged
    ...
    const content = qualificationsContent[lang] || qualificationsContent.EN;
    ...
    res.render('customer/Qualifications', {
        ...
        // ❌ no products here – navbar uses navProducts from middleware
    });

    const elapsed = Date.now() - t0;
    console.log(`🧩 getQualifications controller took ${elapsed} ms`);
};



exports.getLicensePage = (req, res, next) => {
    // Prefer the lang already resolved by middleware
    const langFromMiddleware = res.locals.lang;
    const langFromRoute = (req.params.lang || req.query.lang || 'EN').toUpperCase();
    const lang = (langFromMiddleware || langFromRoute).toUpperCase();
    ...
    const elapsed = Date.now() - t0;
    console.log(`🧩 getLicensePage controller took ${elapsed} ms`);
};
...
exports.getaboutus = (req, res, next) => {
    ...
    const langFromMiddleware = res.locals.lang;
    const langFromParams = req.params.lang?.toUpperCase() || 'EN';
    const lang = langFromMiddleware || langFromParams;
    ...
    const elapsed = Date.now() - t0;
    console.log(`🧩 getaboutus controller took ${elapsed} ms`);
};
